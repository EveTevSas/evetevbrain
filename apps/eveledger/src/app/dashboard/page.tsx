...
function IcoCartera() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>;
}
function IcoAlertaCartera() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>;
}
function IcoTransporte() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8h4l3 3v5h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>;
}
function IcoFaltantes() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>;
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <p style={{ margin: "0 0 0.2rem", fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#4b3075" }}>
            Resumen mensual
          </p>
        ...
        {/* Selector mes/año */}
        <form method="get" style={{ display: "flex", alignItems: "center", gap: "0.5rem", background: "#fff", border: "1px solid #EDF3FA", borderRadius: 12, padding: "0.5rem 0.75rem" }}>
          <select name="mes" defaultValue={mes} style={{ border: "none", background: "transparent", fontSize: "0.85rem", fontWeight: 500, color: "#0A2540", cursor: "pointer", outline: "none" }}>
            {MESES.map((m, i) => (
              <option key={i + 1} value={i + 1}>{m}</option>
            ))}
          </select>
          <select name="anio" defaultValue={anio} style={{ border: "none", background: "transparent", fontSize: "0.85rem", fontWeight: 500, color: "#0A2540", cursor: "pointer", outline: "none" }}>
            {anios.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
          <button type="submit" style={{ background: "#0A2540", color: "#fff", border: "none", borderRadius: 8, padding: "0.35rem 0.875rem", fontSize: "0.8rem", fontWeight: 600, cursor: "pointer" }}>
            Ver
          </button>
        </form>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "1rem" }}>
        {kpis.map(({ etiqueta, valor, full, Ico, iconBg, iconColor, alerta }) => (
          <div key={etiqueta} style={{ background: "#fff", border: "1px solid #EDF3FA", borderTop: `3px solid ${iconColor}`, borderRadius: 14, padding: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem", boxShadow: "0 1px 4px rgba(10,37,64,.04)" }}>
            <p style={{ margin: 0, fontSize: "0.75rem", fontWeight: 600, color: "#334155" }}>
              {etiqueta}
            </p>
            <div>
              <p className="cifra" style={{ margin: 0, fontSize: "clamp(1.4rem, 2vw, 1.8rem)", color: alerta ? "#B91C1C" : "#0A2540", lineHeight: 1.1 }}>
                {valor}
              </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
        ...
        <section style={{ background: "#fff", border: "1px solid #EDF3FA", borderRadius: 14, padding: "1.5rem", boxShadow: "0 1px 4px rgba(10,37,64,.04)" }}>
          <p style={{ margin: "0 0 0.2rem", fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#94A3B8" }}>
            Distribución
          </p>
          ...
          {segmentos.length === 0 ? (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "2rem", flexWrap: "wrap" }}>
              <svg viewBox="0 0 42 42" style={{ width: 160, height: 160, flexShrink: 0 }} role="img" aria-label="Distribución de medios de pago">
                <circle cx="21" cy="21" r="15.9155" fill="none" stroke="#EDF3FA" strokeWidth="7" />
                {segmentos.map((s) => (
                  <circle key={s.clave} cx="21" cy="21" r="15.9155" fill="none"
                ...
                {/* Centro */}
                <text x="21" y="19" textAnchor="middle" fontSize="3.5" fontWeight="700" fill="#0A2540">
              ...
              <ul style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.625rem", listStyle: "none", margin: 0, padding: 0 }}>
                {segmentos.map((s) => (
                  <li key={s.clave} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.75rem" }}>
                    <span style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.82rem", color: "#0A2540" }}>
                      <span style={{ width: 10, height: 10, borderRadius: "50%", background: s.color, flexShrink: 0 }} />
                      {s.etiqueta}
                    </span>
                    <span style={{ fontSize: "0.8rem", color: "#64748B", fontVariantNumeric: "tabular-nums", whiteSpace: "nowrap" }}>
                      {pesosCorto(s.valor)} · <strong style={{ color: "#0A2540" }}>{Math.round(s.pct)}%</strong>
                    </span>
                  </li>
                ))}
              </ul>
                ...
                return (
                  <li key={p.nombre}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "0.375rem" }}>
                      <span style={{ fontSize: "0.875rem", fontWeight: 500, color: "#0A2540" }}>{p.nombre}</span>
                      <span className="cifra" style={{ fontSize: "0.9rem", color: "#64748B" }}>
                        {formatoGalones(p.galones)} <span style={{ fontSize: "0.72rem", fontWeight: 400 }}>gal</span>
                      </span>
                    </div>
                    <div style={{ height: 10, background: "#EDF3FA", borderRadius: 999 }}>
                      <div style={{ height: 10, borderRadius: 999, background: "linear-gradient(90deg, #0a2540, #4b3075)", width: `${pct}%`, transition: "width 0.4s ease" }} />
                    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", alignItems: "stretch" }}>
        ...
        <section style={{ background: "#fff", border: "1px solid #EDF3FA", borderRadius: 14, padding: "1.5rem", boxShadow: "0 1px 4px rgba(10,37,64,.04)" }}>
          ...
          {data.alertas.length === 0 ? (
            <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", background: "#F0FDF4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "0.75rem 1rem" }}>
              <IconoExito className="h-4 w-4 shrink-0 text-eve-exito" />
              <span style={{ fontSize: "0.875rem", color: "#16A34A", fontWeight: 500 }}>Sin alertas este mes. Todo en orden.</span>
            </div>
          ) : (
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <div style={{
              ...
              <p style={{
                ...
                <strong style={{ color: "#fff" }}>Tendencia de ventas:</strong> El volumen de galones
                vendidos muestra una variación del mes. Revisa si hay días con cierres en borrador que
                puedan afectar el consolidado final.
                <br /><br />
                <strong style={{ color: "#fff" }}>Inventario:</strong> Si hay alertas de variación activas,
                considera hacer un conteo físico de verificación antes del cierre de mes para ajustar
                diferencias con la teórica acumulada.
                <br /><br />
                <strong style={{ color: "#22d3ee" }}>Sugerencia:</strong> Los clientes con cartera mayor
                a 90 días tienen mayor riesgo de incobrabilidad. Prioriza contacto directo esta semana.
              </p>
            </div>
            ...
            <p style={{
              ...
              El agente analizará tus datos reales y generará sugerencias personalizadas · Eve Intelligence
            </p>
          </div>
        </div>
      </section>

      </div>{/* fin grid alertas + agente */}

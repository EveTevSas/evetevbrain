  ...
  return (
    <div>
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ margin: "0 0 0.25rem", fontSize: "1.5rem", fontWeight: "700", color: "var(--navy)" }}>Transacciones</h1>
        <p style={{ margin: 0, color: "var(--gray-400)", fontSize: "0.875rem" }}>Historial completo de cobros.</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", alignItems: "flex-end" }}>
          <div>
            <label style={{ display: "block", fontSize: "0.75rem", fontWeight: "600", color: "var(--gray-600)", marginBottom: "0.25rem" }}>Desde</label>
            <input className="input" type="date" value={desde} onChange={(e) => { setDesde(e.target.value); setPage(1); }} style={{ width: 160 }} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: "0.75rem", fontWeight: "600", color: "var(--gray-600)", marginBottom: "0.25rem" }}>Hasta</label>
            <input className="input" type="date" value={hasta} onChange={(e) => { setHasta(e.target.value); setPage(1); }} style={{ width: 160 }} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: "0.75rem", fontWeight: "600", color: "var(--gray-600)", marginBottom: "0.25rem" }}>Estado</label>
            <select className="input" value={estado} onChange={(e) => { setEstado(e.target.value); setPage(1); }} style={{ width: 140 }}>
              {ESTADOS.map((e) => <option key={e} value={e}>{e || "Todos"}</option>)}
            </select>
          </div>
          {(desde || hasta || estado) && (
            <button className="btn-ghost" onClick={() => { setDesde(""); setHasta(""); setEstado(""); setPage(1); }}>
              Limpiar filtros
            </button>
          )}
        </div>
      ...
      <div className="card" style={{ padding: "0 0 0.5rem" }}>
        {loading ? (
          ...
          <div style={{ padding: "3rem", textAlign: "center", color: "var(--gray-400)" }}>
            <p style={{ margin: "0 0 0.5rem", fontSize: "1.1rem" }}>Sin transacciones</p>
            <p style={{ margin: 0, fontSize: "0.85rem" }}>No hay cobros que coincidan con los filtros seleccionados.</p>
          </div>
        ) : (
          <>
            <table>
              ...
              <tbody>
                {data.items.map((cobro: Cobro) => (
                  <tr key={cobro.id}>
                    <td>
                      <span style={{ fontWeight: "600", color: "var(--navy)" }}>{cobro.referencia}</span>
                      <br />
                      <span className="mono" style={{ color: "var(--gray-400)" }}>{cobro.id.slice(0, 8)}…</span>
                    </td>
                    <td style={{ fontWeight: "600" }}>{fmt(cobro.montoMinor, cobro.moneda)}</td>
                    <td>{estadoPill(cobro.estado)}</td>
                    <td>{fmtDate(cobro.creadoEn)}</td>
                    <td>
                      <Link href={`/transacciones/${cobro.id}`} style={{ color: "var(--coral)", fontSize: "0.8rem", textDecoration: "none", fontWeight: "600" }}>
                        Ver →
                      </Link>
                    </td>
                  </tr>

#!/usr/bin/env node
/* Mantiene una sola base de CSS para todas las landings de producto.
 *
 *   node scripts/css-landings.mjs          → copia la fuente a cada landing
 *   node scripts/css-landings.mjs --check  → falla si alguna copia se desvió
 *
...
const fuente = join(raiz, 'packages/brand/landing/base.css');
const destino = 'base.css';

const AVISO = `/* ARCHIVO GENERADO — NO EDITAR.
   Copia de packages/brand/landing/base.css. Edita allí y corre \`pnpm css:sync\`.
 ...
 *  tener que crear antes un base.css vacío ni tocar este script. */
function landings() {
...
const esperado = AVISO + readFileSync(fuente, 'utf8');
...
if (objetivos.length === 0) {
  ...
  console.error('✗ No se encontró ninguna landing con base.css. ¿Se movieron las apps?');
  process.exit(1);
}
...
if (comprobar && desviadas > 0) {
  console.error(
    `\n${desviadas} copia(s) desviada(s). Edita packages/brand/landing/base.css` +
      ' y corre `pnpm css:sync`.',
  );
  process.exit(1);
}
console.log(comprobar ? `✓ ${objetivos.length} landing(s) al día` : '✓ sincronizado');

...
navegador (widget)                apps/rag-assistant           Model Studio · Moonshot
      ...
      │                                   │        │ pasa                     │
      │                                   │ 6 GENERACIÓN ────────────────────►│ kimi-k2.6, tope 220 tokens
      │◄═════ SSE, token a token ═════════│◄─────────────────────────────────│
...
| 3   | **Prompt de anclaje**       | `base/_sistema.md`       | Tope de 220 tokens, respuesta de máximo tres frases, cita obligatoria, y la instrucción de decir «no lo tengo» sin adornar. Versionado y revisado por PR como cualquier otro documento. **La temperatura se cayó de esta lista al implementar:** `kimi-k2.6` solo admite la suya, y no pasó nada — el anclaje nunca dependió de un parámetro de muestreo. |
